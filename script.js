...
const getChart = async () =>{
    ...
    try{
        const response = await fetch('https://api.binance.us/api/v3/klines?symbol=ETHUSDT&interval=1d&limit=1000')
        if(response.ok){
...
const comparison =(price, emaPrice) =>{
    ...
    if(price<emaPrice){
        comparisonDom.innerHTML = `assets price is <span style="color:red"><b>below</b></span> the 50EMA. A downtrend is expected. Play at your own risk.`
    } else {
        comparisonDom.innerHTML = `assets price is <span style="color:green"><b>above</b></span> the 50EMA. Asset should be longed. Keep risk management in mind.`
    }
